...
function setupPresence(roomName, user) {
  ...
  presenceRef
    .set(user.nickname)
    .catch((e) =>
      console.error(`[Presence ${roomName}] Error set presence:`, e)
    );
...
async function setupJoin(roomName, announce = false) {
  ...
  const messageListener = roomChatRef.limitToLast(100).on(
    "child_added",
    (snapshot) => {
      ...
      if (!messageCache[roomName].some((m) => m.id === message.id)) {
        if (messageCache[roomName].length > 100) {
  ...
  const presenceListener = roomPresenceRef.on(
    "child_removed",
    (snapshot) => {
  ...
  for (const roomName in activeListeners) {
    ...
    if (roomListeners.messages) {
      database
        ...
        .off("child_added", roomListeners.messages);
    }
    if (roomListeners.presence) {
      database
        .ref(`rooms/${roomName}/presence`)
        .off("child_removed", roomListeners.presence);
    }
  }
  activeListeners = {};
  messageCache = {};
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  (async () => {
    ...
    switch (request.type) {
      ...
      case "REGISTER":
        try {
          ...
          if (pass !== confirm) {
            sendResponse({ success: false, error: "As senhas não coincidem." });
            return;
          }
          if (pass.length < 4) {
            sendResponse({
          ...
          if (nick.length < 3 || nick.length > 12) {
            sendResponse({
              success: false,
              error: "O nick deve ter entre 3 e 12 caracteres.",
            });
            return;
          }
          ...
          if (snapshot.exists()) {
            sendResponse({
              success: false,
              error: "Este nick já está em uso.",
            });
          } else {
            const newUserId =
    ...
    if (!userInfo.nickname || !userInfo.userId) {
      sendResponse({
        success: false,
        error: "Você precisa estar logado. Use /login",
      });
      return true;
    }
    ...
    switch (request.type) {
      ...
      case "GET_ROOM_MEMBERS":
        try {
          ...
          const allUsersWithRoomRef = usersRef
            .orderByChild(`rooms/${roomName}`)
            .equalTo(true);
          ...
          const onlineNicks = presenceSnapshot.exists()
            ? Object.values(presenceSnapshot.val())
            : [];
          const onlineNickSet = new Set(
            onlineNicks.map((n) => n.toLowerCase())
          );
          ...
          if (allUsersSnapshot.exists()) {
            allUsersSnapshot.forEach((userSnapshot) => {
              ...
              if (onlineNickSet.has(userNick.toLowerCase()))
        ...
        try {
          ...
          if (!snapshot.exists()) {
            if (isPublic && password === "123") {
              ...
              await usersRef
                .child(userInfo.nickname.toLowerCase())
                .child("rooms")
                .child(roomName)
                .set(true);
              sendResponse({
      ...
      case "LEAVE_ROOM":
        ...
        if (roomListeners && userInfo.userId) {
          ...
          if (roomToLeave.includes("_&_")) {
            ...
            const otherNick =
              names[0].toLowerCase() === userInfo.nickname.toLowerCase()
                ? names[1]
                : names[0];
            await usersRef
              .child(otherNick.toLowerCase())
              .child("rooms")
              .child(roomToLeave)
              .remove();
          }
          ...
          await database
            .ref(`rooms/${roomToLeave}/presence/${userInfo.userId}`)
            .remove();
          if (roomListeners.messages) {
            try {
      ...
      case "SEND_MESSAGE":
        ...
        const roomRef = activeListeners[msgRoom]?.messages
          ? database.ref(`rooms/${msgRoom}/messages`)
          : null;
        if (roomRef) {
          const messageToSend = {
            ...message,
      ...
      case "UPDATE_USER_INFO":
        ...
        if (userInfo.nickname) {
          usersRef
            ...
            .update(updates)
            .catch((error) =>
              console.error("Error updating FB profile:", error)
            );
        }
...
function initialize() {
    ...
    chrome.storage.sync.get(["loggedInUser"], async (result) => {
      ...
      if (result.loggedInUser) {
        ...
        if (snapshot.exists()) {
            ...
            await Promise.all(
              roomNames.map(async (roomName) => {
                try {
                  ...
                  console.warn(
                    `[BG] Error re-joining room ${roomName}:`,
                    e.message
                  );
          ...
          console.warn(
            `[BG] User ${loggedInNick} in storage but not Firebase. Logging out.`
          );
          await performLogout();
          resolve();
        }
  ...
  initializationPromise
    .then(() => {
      console.log(
        "[BG] Initialization check complete. Current user:",
        userInfo.nickname
      );
    })
    .catch((error) => {
